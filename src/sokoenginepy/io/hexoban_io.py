...
from ..common import Characters, index_1d, index_x, index_y
...
class HexobanTextConverter:
    def convert_to_string(
        ...
        floor = Characters.VISIBLE_FLOOR if use_visible_floor else Characters.FLOOR
        ...
        for row in range(0, height):
            ...
            for col in range(0, width):
                ...
                line += floor
                if Characters.is_empty_floor(puzzle_char):
                    line += floor
                else:
                    line += puzzle_char
    ...
    def _find_rightmost_non_floor(strings: List[str]) -> Optional[int]:
        def rightmost_finder(strings: List[str], row_parity: int):
            ...
            for row in range(row_parity % 2, height, 2):
                for col in range(0, width):
                    if not Characters.is_empty_floor(strings[row][col]):
                        cell_found = True
    ...
    def convert_to_internal(self, board: str) -> Tuple[List[str], bool]:
        ...
        if even_row_x_parity < 0 or odd_row_x_parity < 0:
            internal = height * [int(width / 2) * Characters.VISIBLE_FLOOR]
        ...
        if layout_ok:
            internal = PuzzleParser.normalize_width(internal, Characters.VISIBLE_FLOOR)
        ...
        if height != 0 and width != 0:
            ...
            for i in range(0, height):
                ...
                has_non_floor_left_in_odd_row = has_non_floor_left_in_odd_row or (
                    i % 2 == 1 and not Characters.is_empty_floor(parsed[i][0])
                )
    ...
    def add_column_left(strings: List[str]) -> List[str]:
        return [Characters.VISIBLE_FLOOR + line for line in strings]

    @staticmethod
    def add_column_right(strings: List[str]) -> List[str]:
        return [line + Characters.VISIBLE_FLOOR for line in strings]

    @staticmethod
    def add_row_top(strings: List[str]) -> List[str]:
        w = HexobanPuzzleParser.calculate_width(strings)
        return [w * Characters.VISIBLE_FLOOR] + strings
    ...
    def _find_first_non_floor(strings: List[str]):
        ...
        for row in range(0, height):
            ...
            for column in range(0, width):
                ...
                if not Characters.is_empty_floor(normalized[row][column]) and (
                    column > x or row > y
                ):
                    x = column
                    y = row
