...
namespace sokoengine {
  namespace io {
    ...
    namespace sok_rle {
      ...
      LIBSOKOENGINE_LOCAL bool _parse(
        iterator_type         &iter,
        iterator_type          end,
        sok_rle::ast::RleData &into,
        std::ostream          &err
      ) {
      ...
      //
      // Parses Sokoban RLE expression (board or snapshot) and produces JSON
      // representation of AST. Prints JSON AST to dest.
      //
      // This one is mainly for experimentation and grammar debugging purposes.
      //
      LIBSOKOENGINE_LOCAL bool
      parse_to_ast(const std::string &src, std::ostream &dest, std::ostream &err) {
      ...
      //
      LIBSOKOENGINE_LOCAL bool
      parse_to_eval(const std::string &src, std::ostream &dest, std::ostream &err) {
        iterator_type         iter = src.begin();
        iterator_type         end  = src.end();
        sok_rle::ast::RleData ast;
    ...
    string Rle::encode(const string &line) {
      ...
      if (any_of(to_encode.begin(), to_encode.end(), [](char c) {
            return isdigit(c);
          })) {
      ...
      while (next_found != string::npos) {
        if (next_found - found > 1) {
          oss << next_found - found;
        }
        oss << (to_encode[found] == '\n' ? EOL : to_encode[found]);
        found      = next_found;
        next_found = to_encode.find_first_not_of(to_encode[found], found);
