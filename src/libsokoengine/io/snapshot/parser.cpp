  ...
  namespace io {
    namespace snapshot_parsing {
      ...
      LIBSOKOENGINE_LOCAL bool _parse(
        iterator_type &iter, iterator_type end, ast::Snapshot &into, ostream &err
      ) {
    ...
    auto const parser =
      ...
      parse_to_ast(const string &src, ostream &dest, ostream &err) {
      ...
      //
      LIBSOKOENGINE_LOCAL bool
      parse_to_eval(const string &src, SnapshotData &dest, ostream &err) {
        iterator_type iter = src.begin();
        iterator_type end  = src.end();
        ast::Snapshot ast;
