...
private:
  ...
  preparse_results_t preparse_board(const string &board_string) const {
    ...
    if (has_non_floor_left_in_odd_row) {
      for (string &line : parsed)
        line.insert(line.begin(), BoardCell::FLOOR);
      width += 1;
    }
  ...
  position_t find_first_non_floor(const StringList &list) const {
    ...
    for (position_t row = 0; row < height && !non_floor_found; ++row) {
      for (position_t col = 0; col < width && !non_floor_found; ++col) {
        if (!BoardCell::is_empty_floor_chr(normalized[row][col]) &&
            (col > x || row > y)) {
          x = col;
          y = row;
  ...
  bool is_type2(const StringList &list) const { return !is_type1(list); }
};
...
string HexobanBoardPrinter::print(const VariantBoard &board, bool use_visible_floor,
  ...
  return HexobanTextConverter().convert_to_string(board, use_visible_floor, rle_encode);
}
