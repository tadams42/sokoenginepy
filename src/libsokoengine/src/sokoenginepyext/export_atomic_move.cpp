  ...
  py::class_<AtomicMove>(m, "AtomicMove")
      ...
      .def(py::pickle(
          [](const AtomicMove &self) { // __getstate__
            return py::make_tuple(self.direction(), self.is_push_or_pull(),
                                  self.is_jump(), self.is_pusher_selection(),
      ...
      .def_property("moved_box_id",
                    ...
                    [](AtomicMove &self, const py::object &val) {
                      if (val.is_none())
                        self.set_moved_box_id(NULL_ID);
                      else
                        self.set_moved_box_id(val.cast<piece_id_t>());
                    })
      ...
      .def_property("pusher_id",
                    ...
                    [](AtomicMove &self, const py::object &val) {
                      if (val.is_none())
                        self.set_pusher_id(NULL_ID);
                      else
                        self.set_pusher_id(val.cast<piece_id_t>());
                    })
