...
public:
  ...
  explicit BenchmarkType(EBenchmarkType which)
    : m_type(which) {}
...
public:
  MovementBenchmark(
    BoardType board_type, BenchmarkType benchmark_type, size_t moves_count
  )
...
int run_benchmarks(double pivot_speed) {
  sokoengine::benchmarks::MovementBenchmarkPrinter::run_all(pivot_speed);
