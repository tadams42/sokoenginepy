...
public:
  ...
  /// @param is_pusher_selection marks this step as part of pusher selecting sequence on
  ///        boards with multiple pushers (Multiban game variant)
  /// @param pusher_id ID of pusher that performed movement. Pusher usually gets it's ID
  ///        from BoardManager
  /// @param is_current_pos flags this step as current position in game snapshot
  ///
  /// @throws std::invalid_argument single step can only be one thing at a time: `move`,
  ///         `push/pull` or `jump`.
  ...
  ///
  /// Pusher ID must always be valid. When setting it to invalid value, PusherStep will
  /// silently set pusher ID to Config::DEFAULT_ID.
  ///
  ...
  ///
  /// True if this move is part of jump sequence in :attr:`.SolvingMode.REVERSE` games.
  ///
  bool is_jump() const;
