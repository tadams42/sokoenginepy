...
public:
  ...
  void reinit(const string &board_str, bool reconf_edges) {
    if (!io::is_blank(board_str)) {
      Strings board_rows = m_tessellation->parser().parse(board_str);
...
string VariantBoard::to_str(bool use_visible_floor) const {
  return m_impl->m_tessellation->printer().print(*this, use_visible_floor);
}

string VariantBoard::str() const { return to_str(false); }
...
size_t VariantBoardParser::calculate_width(const Strings &string_list) {
  size_t width = 0;
...
Strings VariantBoardParser::normalize_width(const Strings &strings, char fill_chr) {
...
Strings VariantBoardParser::cleaned_board_lines(const std::string &line) {
  if (io::is_blank(line)) { return Strings(); }
...
Strings VariantBoardParser::parse(const string &board_str) const {
  return cleaned_board_lines(board_str);
}

string VariantBoardPrinter::print(const VariantBoard &board,
