...
from sokoenginepy.exceptions import SnapshotConversionError
...
class DescribeSnapshotStringParser:
    ...
    class Describe_convert_from_string:
        def it_replaces_internal_data_with_atomic_moves_from_string(
            ...
            assert sokoban_snapshot[0] == AtomicMove(
                Direction.LEFT, box_moved=True
            )
            assert sokoban_snapshot[1] == AtomicMove(
                Direction.UP, box_moved=True
            )
            assert sokoban_snapshot[2] == AtomicMove(
                Direction.RIGHT, box_moved=True
            )
            assert sokoban_snapshot[3] == AtomicMove(
