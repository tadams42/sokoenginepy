...
class DescribeHashedBoardManager:
    def it_hashes_board_layout(self, hashed_board_manager):
        assert hashed_board_manager.boxes_layout_hash is not None
        assert hashed_board_manager.boxes_and_pushers_layout_hash is not None
        assert hashed_board_manager.boxes_layout_hash != hashed_board_manager.boxes_and_pushers_layout_hash

    def test_moving_box_modifies_hashes_consistently(self, hashed_board_manager):
        initial_box_hash = hashed_board_manager.boxes_layout_hash
        initial_pus_hash = hashed_board_manager.boxes_and_pushers_layout_hash
        ...
        hashed_board_manager.move_box_from(
    ...
    def test_switching_boxes_and_goals_preserves_position_hashes(
        ...
        initial_hash = hashed_board_manager.boxes_layout_hash
        hashed_board_manager.switch_boxes_and_goals()
        after_switch_hash = hashed_board_manager.boxes_layout_hash
        assert initial_hash != after_switch_hash

        hashed_board_manager.switch_boxes_and_goals()
        assert hashed_board_manager.boxes_layout_hash == initial_hash
        hashed_board_manager.switch_boxes_and_goals()
        assert hashed_board_manager.boxes_layout_hash == after_switch_hash
        hashed_board_manager.switch_boxes_and_goals()
        assert hashed_board_manager.boxes_layout_hash == initial_hash
