...
from sokoenginepy.game import AtomicMove, Direction, Snapshot, SolvingMode, Tessellation
...
def reverse_sokoban_snapshot():
    return Snapshot.from_data("lurdLURD{lurd}[lurd]LURD")
...
def sokoban_snapshot():
    return Snapshot.from_data(
        tessellation_or_description=Tessellation.SOKOBAN, data="lurdLURD{lurd}LURD"
    )
...
class DescribeGameSnapshot:
    ...
    class Describe_init:
        ...
        def it_parses_solving_mode_from_moves_data(self):
            assert (
                Snapshot.from_data(
                    tessellation_or_description=Tessellation.SOKOBAN, data="[lurd]"
                ).solving_mode
                == SolvingMode.REVERSE
            )
            assert (
    ...
    class Describe_parsing:
        def it_ignores_spaces_and_current_position_character(self):
            snapshot = Snapshot.from_data("  \n *  \t l ", Tessellation.SOKOBAN)
            assert snapshot.solving_mode == SolvingMode.FORWARD
            assert snapshot._moves == [AtomicMove(Direction.LEFT)]

        def it_accepts_blank_input_as_empty_forward_snapshot(self):
            snapshot = Snapshot.from_data("  \n   \t  ", Tessellation.SOKOBAN)
            assert snapshot.solving_mode == SolvingMode.FORWARD
            assert snapshot._moves == []

        def it_fails_on_non_snapshot_characters(self):
            with pytest.raises(ValueError):
                Snapshot.from_data("ZOMG! ", Tessellation.SOKOBAN)

        def it_sets_mode_to_reverse_if_jumps_are_found(self):
            snapshot = Snapshot.from_data("[lurd] ", Tessellation.SOKOBAN)
            for atomic_move in snapshot._moves:
                assert atomic_move.is_jump

        def it_ignores_empty_jump_and_pusher_selection_sequences(self):
            snapshot = Snapshot.from_data("[]lurd", Tessellation.SOKOBAN)
            assert len(snapshot._moves) == 4

        def it_detects_reverse_snapshot_while_ignoring_empty_jumps(self):
            snapshot = Snapshot.from_data("[]lurd", Tessellation.SOKOBAN)
            assert snapshot.solving_mode == SolvingMode.REVERSE

        def it_fails_on_rle_errors(self):
            with pytest.raises(ValueError):
                Snapshot.from_data("((4l)", Tessellation.SOKOBAN)

        def it_fails_on_non_matched_sequence_separators(self):
            with pytest.raises(ValueError):
                Snapshot.from_data("[lurd", Tessellation.SOKOBAN)

        def it_correctly_detects_jumps(self):
            snapshot = Snapshot.from_data("[lurd] ", Tessellation.SOKOBAN)
            for atomic_move in snapshot._moves:
                assert atomic_move.is_jump

        def it_correctly_detects_pusher_selections(self):
            snapshot = Snapshot.from_data("{lurd} ", Tessellation.SOKOBAN)
            for atomic_move in snapshot._moves:
                assert atomic_move.is_pusher_selection

        def it_fails_on_moves_illegal_in_context_of_requested_tessellation(self):
            with pytest.raises(ValueError):
                Snapshot.from_data("Nlurd", Tessellation.SOKOBAN)

        def it_fails_on_jumps_that_contain_pushes(self):
            with pytest.raises(ValueError):
                Snapshot.from_data("lu[rD]", Tessellation.SOKOBAN)

        def it_fails_on_pusher_selections_that_contain_pushes(self):
            with pytest.raises(ValueError):
                Snapshot.from_data("lu{rD}", Tessellation.SOKOBAN)

        def it_creates_snapshot_when_parsing_is_successful(self):
            sokoban_snapshot = Snapshot.from_data("LURD")
        ...
        def it_raises_on_parsing_errors(self):
            with pytest.raises(ValueError):
                Snapshot.from_data("42")
